...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if ((!cv.firstName && !cv.lastName) || cv.yearsOfExperience <= 1) result.rejectedApplicants++;
        else result[cv.tech]++
    }

    return {
        python_devs: result.Python,
        javaScript_devs: result.JS,
        dotNet_devs: result['.Net'],
        java_devs: result.Java,
        totalApplicants: result.totalApplicants,
        rejectedApplicants: result.rejectedApplicants,
    }
};
// -------------------------------------------------------------------------------------------------------
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let i = 0; i < grade.classes.length; i++) {
            const schoolClass = grade.classes[i];

            let sum = 0;
            for (let i = 0; i < schoolClass.classScores.length; i++) {
                const score = schoolClass.classScores[i];
                sum += score
            }
            const classAvg = sum / schoolClass.classScores.length;

            schoolClass.avg = Math.floor(classAvg);
        }
    }
