...
const cvsFiltration = (arr) => {
    // write your code here
    const filteredCv = arr.filter(cv => {
        return cv.yearsOfExperience > 4 && cv.tech === 'JS'
    })
    return filteredCv.map(cv => {
        return {
            fullName: `${cv.firstName || ''} ${cv.LastName || ''}`.trim(),
            tech: cv.tech
        }
    })
}
...
const vowelsFiltration = (arr) => {
    // write your code here
    return arr.filter(v => !/(a|e|o|i|u)/gi.test(v))
}
// -------------------------------------------------------------------------------------------------------
...
const skillComparison = (arr1, arr2) => {
    // write your code here
    const diffArr1 = arr1.filter(v => !arr2.includes(v))
    const diffArr2 = arr2.filter(v => !arr1.includes(v))
